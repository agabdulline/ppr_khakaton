...
def process_reviews_3months():
    ...
    with open(f"../parsed/org_done/qqq.json", "r", encoding="utf-8") as f:
        reviews = json.load(f)
    plus = reviews['3month']['all_plus']
    minus = reviews['3month']['all_minus']
    # for rev in reviews:
    #     if three_months_ago_timestamp > rev['date']:
    #         continue
    #     rating = rev["rating"]
    #     text = rev["text"]
    #     messages = [
    ...
    messages = [
        {"role": "system",
         "content": f"Ты должен помочь мне проанализировать списки сильных и слабых сторон, "
                    f"выявленных из отзывов моих клиентов и выявить по 3 преимуществa и недостаткa моего продукта, "
                    ...
                    f"каждое из которых - массив из 3 строк. Больше ничего в ответе не пиши. \n "
    ...
    with open(f"../parsed/org_done/processed_3month_last.json", "w", encoding="utf-8") as f:
        json.dump(processed_reviews, f, ensure_ascii=False, indent=2)
    return processed_reviews

process_reviews_3months()